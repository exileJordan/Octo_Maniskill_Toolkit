...
flags.DEFINE_integer(
    "num_val_batches",
    6,
    "Number of validation batches to average.",
)
...
def main(_):
    ...
    wandb.init(name="finetune_maniskill_200", project="octo")
    ...
    dataset = make_single_dataset(
        dataset_kwargs=dict(
            name="maniskill_pickcube_dataset",
            ...
            standardize_fn=ModuleSpec.create(
                ...
                "pickcube_proprio_qpos_qvel_tcp_pose"
            ),
        ),
    ...
    # add val_dataset,can be deleted
    ...
    val_data_iter = map(process_batch, val_data_iter) #val_data_iter, can be deleted
    ...
    for i in tqdm.tqdm(range(5000), total=5000, dynamic_ncols=True): # adjust number of steps as needed, originally 5000.
        ...
        if (i + 1) % 100 == 0:
            ...
            for _ in range(FLAGS.num_val_batches):
                val_batch = next(val_data_iter)
                val_info = val_step(train_state, val_batch)
                val_infos.append(jax.device_get(val_info))

            val_info = jax.tree_util.tree_map(
        ...
        if (i + 1) % 500 == 0: # original is 1000, adjust as needed
            # save checkpoint
